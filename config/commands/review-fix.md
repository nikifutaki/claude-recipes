...
1. **Skill 呼び出し (`/review` 実行)**
    ...
    - **Skill 呼び出しが失敗した場合の振る舞い**: ステップ 1 の `Skill(skill="review", args="<PR番号>")` 呼び出し自体が失敗した場合（`/review` 未インストール、スキル内部エラー、タイムアウト等）、Reviewer は `review_{n}_{m}.md` を**書き出さず**、Task ツール応答としてエラー内容（Skill 失敗の旨と stderr / エラーメッセージ等）を Coordinator に返す。リトライや空ファイル生成は行わず、リトライ判断は Coordinator 側の「部分的失敗時の振る舞い」（「ロールの実行モデル」セクション参照）に委ねる。
...
- **(b) PR 情報取得 + 参照可否・種別・state・タイトル取得**
    - `gh pr view <PR番号> --json isCrossRepository,state,headRefName,title` を実行し、`isCrossRepository` / `state` / PR ブランチ名 `<PR ブランチ名>` / PR タイトルをまとめて取得する（※ 可読性のため本節以降では論理的に 4 フィールドをステップ分割して説明するが、実装上は上記のように 1 コールに集約する）。
...
- **(c) ローカル clean 検証**
    - (c-1) `git symbolic-ref --short HEAD` の出力が (b) で取得した `<PR ブランチ名>` と一致すること。
    ...
    - (c-2) `git fetch origin "<PR ブランチ名>"` で remote-tracking ref を最新化する。
        - ※ `git fetch origin "<PR ブランチ名>"` は local の `refs/remotes/origin/<PR ブランチ名>` を更新する**副作用のあるコマンド**である（**通常設定下では本コマンドで更新されるのは主に `refs/remotes/origin/<PR ブランチ名>` のみ。`fetch.pruneTags` や `remote.origin.fetch`（refspec 拡張）、`remote.origin.tagOpt=--tags`、`remote.<name>.prune=true` 等の特殊設定下では他の ref（tag や削除済み ref の pruning 等）も影響を受ける可能性がある**）。「Preflight は参照のみ」と誤解しないこと。
        - この fetch が無いと、別環境からの push 後に `origin/<PR ブランチ名>` が古いまま残り、後続の `rev-list` が「未 push コミットあり」と誤判定して Preflight が不当に abort する。
    - (c-3) 以下の**両方**のコマンドが **exit code 0 で終了し、かつ stdout が空** であること（**exit code 0 + stdout 空** を成功条件とする; exit code 非 0 の場合は本セクション冒頭の「外部コマンド自体の失敗時ハンドリング（Preflight 全ステップ共通）」の規約により即中断する — ref 解決失敗・壊れた repo 状態等が該当）。`git status --porcelain` についても同等の規約で統一して扱う:
...
- **(d) タイムスタンプディレクトリ作成（上記 (a)〜(c) が全て PASS した後にのみ実行）**
    ...
    - `<REVIEW_DIR>` = `<REPO_ROOT>/.claude/reviews/<PR番号>/<YYYYMMDD_HHMMSS>` の形で**絶対パスとして構築**する（**末尾スラッシュなし**）。後続の参照 `<REVIEW_DIR>/review_{n}_{m}.md` 等は `<REVIEW_DIR>` と `/` の間に明示的なスラッシュを置く形になり、末尾スラッシュ付き定義との二重スラッシュを避ける。`mkdir -p <REVIEW_DIR>` は末尾スラッシュなしでも正常に動作する。
...
- **Implementer 起動時の入力**: Implementer サブエージェントを起動する際は、`<REVIEW_DIR>/task_{n}.md` の絶対パス・`<REVIEW_DIR>/review_{n}_*.md` の絶対パス・PR 番号・PR ブランチ名（Preflight (b) で取得した `<PR ブランチ名>`）・**PR タイトル**（Preflight (b) の集約コールで取得済み）をプロンプトに含めて渡す。PR タイトルは Implementer がコミットメッセージの `<spec 名>` を抽出するために使用する（「コミットポリシー → spec 名の決定ルール」参照）。Implementer 側での `gh pr view` 再呼び出しを避けるため、Coordinator が取得済みの branch 名・タイトルを再利用する。
...
#    - `gh pr view <PR番号> --json isCrossRepository,state,headRefName,title` を 1 コールで実行し、
#      `isCrossRepository` / `state` / `headRefName` / `title` を同時取得する（4 フィールドを 1 コールに集約）。
...
#    .claude/reviews/<PR番号>/<YYYYMMDD_HHMMSS> を作成（末尾スラッシュなし定義）。過去のディレクトリは削除しない。
...
#    PR ブランチ名・**PR タイトル**（いずれも Preflight (b) の集約コールで取得済み）を渡す。
#    Implementer の手順:
