...
本コマンドのスラッシュコマンド呼び出し（例: `/review-fix claude=2 codex=2`）の引数文字列は、Claude Code ハーネスによって `$ARGUMENTS` として Coordinator のプロンプト中に展開される。Coordinator は `$ARGUMENTS` に入った生の文字列を字句解析（下記の引数バリデーションルール（字句解析・値域・既定値の各箇条書き）に従う）・バリデーションし、Step 0 の判断を行う。引数が空文字または空白のみなら「引数なし」扱いとし、既定値 `claude=2 codex=0` を適用する。
...
- `codex review` は対象ファイルと git 状態を **読み取り専用** で参照するため、`codex=M>=2` で複数プロセスが同一作業ディレクトリ上で並列に走っても互いに干渉しない（codex 0.121.0 の挙動に依拠）。
...
2. **期待された出力ファイル（Claude: `review_{n}_{m}.md`、codex: `review_{n}_c{k}.md`）がそもそも存在しない**（`test -f` 相当でファイルが見つからない）→ 当該 Reviewer は失敗扱い（`.failed` リネームは存在しないファイルに対しては不要）。存在しないことを Coordinator 内部で記録し、「部分的失敗時の振る舞い」の成功件数カウントに反映する。
3. **出力が空（0 バイト）**、または **finding bullet（`[P0]` / `[P1]` / `[P2]` / `[P3]`）も Severity 見出し（`### Critical` / `### High` / `### Medium` / `### Low` / `### Nit`）も全く含まず、かつ `Approve` / `Request Changes` / `Comment` の literal 文字列もどこにも含まれない** → 実質空レビューとして破棄。
4. 失敗・不適合の場合は、当該 `.md` ファイルを **`.failed` 拡張子にリネーム**（例: `review_{n}_c{k}.md.failed` / `review_{n}_{m}.md.failed`）する。これにより `review_{n}_*.md` glob の対象から外れる。
5. codex の stderr は `review_{n}_c{k}.err` に退避し、デバッグ用に保持する（成功・失敗を問わず残してよい）。
6. 統合時は **ファイル拡張子が厳密に `.md` で終わるもののみ** を対象とする（`.md.failed` が緩い glob で誤って拾われることを避けるため）。
7. 上記のうちいずれかを満たした Reviewer については「部分的失敗時の振る舞い」ルールに従う。

**具体的なリネーム手順（共通ルールの運用ステップ）:**

- **codex Reviewer**: Bash ツールの終了コードおよび上記 (3) の意味的条件を確認し、該当すれば `mv <REVIEW_DIR>/review_{n}_c{k}.md <REVIEW_DIR>/review_{n}_c{k}.md.failed` 相当を実行する（`<REVIEW_DIR>` / `{n}` / `{k}` は実体の絶対パスリテラルと整数に置換する）。ただし上記 (2) により出力ファイルがそもそも存在しない場合は `.failed` リネームをスキップする。
- **Claude Reviewer**: Coordinator は **Task ツール戻り値のステータスを確認** し、エラー／タイムアウト、または上記 (3) の意味的条件に該当する場合、Bash ツールで `test -f <REVIEW_DIR>/review_{n}_{m}.md && mv <REVIEW_DIR>/review_{n}_{m}.md <REVIEW_DIR>/review_{n}_{m}.md.failed` 相当を実行し、当該出力ファイル（生成されていれば）を `.failed` にリネームする。削除は行わず `.failed` リネームで統一する（上記 (2) により出力ファイルが存在しない場合は `test -f` が偽となりリネームはスキップされる）。
...
# Step 0.6. 作業ディレクトリ precondition 検証
#    Step 0.6 は **`codex=M>=1` の場合のみ実行** する。M=0（Claude only 構成）では
#    本 Step をスキップして Step 1 へ進む（Claude Reviewer は作業ツリー状態を直接参照しないため）。
#    `codex=M>=1` のときは以下を Coordinator が検証し、いずれか満たされなければサイクルを中断する：
