...
> **Note:** レビューアーティファクトはプロジェクト内の `.claude/reviews/` 配下に保存される。このコマンドを任意のリポジトリで使用する場合、**レビュー対象のリポジトリ**の `.gitignore` に `.claude/reviews/` を追加して、アーティファクトがリポジトリにコミットされないようにすること。
