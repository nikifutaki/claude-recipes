...
- **(b) PR 情報取得 + 参照可否・種別・state・タイトル取得**
    ...
    - **パース方式の統一規約:** `gh pr view --json field1,field2,...` で集約取得した結果から各フィールドを抽出する際は、`-q` オプションで jq クエリを渡す方式に統一する（`gh` CLI の `-q` は jq 構文をそのままサポートする）。単一フィールド取得なら `-q .fieldName`、複数フィールドの一括抽出なら `-q '[.isCrossRepository, .state, .headRefName, .title] | @tsv'` のように TSV で取り出す等、用途に応じて jq クエリを組み立てる。同じ spec 内で「`--json field1,field2`（複数フィールド取得のみ）」と「`--json field -q .field`（`-q` 併用の単一抽出）」が混在するとパース方式のドリフトを招くため、本 spec 内の `gh pr view` 呼び出しは原則 `--json` + `-q` の組み合わせに揃える（抽出不要で JSON 全体を扱う稀なケースは除く）。
...
- **コミットメッセージ形式**: 1 行目（subject）に**ラウンド番号を必ず含める**。推奨形式は `Address Round {n} review feedback on <spec 名> spec`（`<spec 名>` は対象 PR から決定する — 下記「spec 名の決定ルール」参照）。例: PR タイトルが `Refactor /review-fix to take PR number and delegate to /review skill` なら `<spec 名>` は `/review-fix` となり、コミットメッセージは `Address Round {n} review feedback on /review-fix spec` となる。これにより後から `git log --grep "Round {n}"` でラウンド単位の追跡が可能となり、かつ本 skill を将来別 spec に対して適用する場合も形式が揃う。
    - **spec 名の決定ルール（以下の順序で決定的に適用する）**:
        ...
        2. **主たる変更ファイルのパス名から抽出**: 上記 1 が該当しない場合、PR の主たる変更対象ファイル（`config/commands/<name>.md` パターンで**最も多く変更された** `<name>`。変更行数ではなくファイル数ではなく、パターンに合致する複数ファイルがある場合はファイル名の辞書順で最初のもの）の `<name>` を採用し、`/<name>` の形式とする（例: `config/commands/review-fix.md` → `/review-fix`）。
...
- **PR 番号**（引数として受け取ったもの）
- **PR ブランチ名**（Coordinator の Preflight (b) で `gh pr view <PR番号> --json isCrossRepository,state,headRefName,title` で 4 フィールドをまとめて取得した後、別途 `-q .headRefName` クエリで `headRefName` を抽出するか、1 コールで `-q '[.isCrossRepository, .state, .headRefName, .title] | @tsv'` のような TSV 出力を使って全フィールドを一括抽出した経路で取得済みのものをそのまま渡す。**`--json` の後に複数フィールドを列挙した状態で `-q .headRefName` を併用すると `.headRefName` しか抽出されず他のフィールド情報が失われるため、複数フィールド情報を維持する目的では単一フィールド jq 抽出と両立しない**。Implementer 側での `gh pr view` 再呼び出しは行わず、渡された branch 名を `git symbolic-ref --short HEAD` の出力と照合して防御的に検証するのみとする。詳細は「各ロールの責務 → Implementer」セクション参照）
- **PR タイトル**（Coordinator の Preflight (b) の集約コールで `title` フィールドとして取得済みのものをそのまま渡す。Implementer はコミットメッセージの `<spec 名>` を PR タイトルから抽出するために使用する。詳細は「各ロールの責務 → Implementer → コミットポリシー → spec 名の決定ルール」参照）
