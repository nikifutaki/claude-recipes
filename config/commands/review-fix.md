...
- **(c) ローカル clean 検証**
    - (c-1) `git symbolic-ref --short HEAD` の出力が (b) で取得した `<PR ブランチ名>` と一致すること。
        - 非 0 終了の場合は「HEAD が branch を指していない（detached HEAD または repo 異常 — 例: `.git` 破損、HEAD 欠損、リポジトリでない作業ディレクトリからの実行）のため `gh pr checkout <PR番号>` で PR ブランチに切り替えてから再実行してください。`git symbolic-ref --short HEAD` の stderr: `<stderr 全文>`」のエラーで即中断する。`git symbolic-ref --short HEAD` は detached HEAD 以外（`.git` ディレクトリ破損、HEAD ref 欠損、リポジトリ外での実行等）でも非 0 で終了するため、エラーメッセージでは「detached HEAD と断定しない」形式にし、併せて stderr を表示して原因特定を支援する。
    ...
    - (c-2) `git fetch origin <PR ブランチ名>` で remote-tracking ref を最新化する。
        - ※ `git fetch origin <PR ブランチ名>` は local の `refs/remotes/origin/<PR ブランチ名>` を更新する**副作用のあるコマンド**である（その他の local branch 状態には影響しない）。「Preflight は参照のみ」と誤解しないこと。
...
- **`title` が空文字列または null の場合のフォールバック**: Preflight (b) で取得した `title` が空文字列または `null` の場合、Coordinator は警告ログを出力する（ユーザー向けレポートまたは `task_{n}.md` の冒頭注記として「PR タイトルが空または null のため spec 名抽出が省略される」を明示）。その上で Implementer には空 title をそのまま渡す。Implementer 側では spec 名決定ルール step 1（スラッシュプレフィックスマッチ）が失敗し、step 3（`<spec 名>` 省略 = subject が `Address Round {n} review feedback` のみ）に自動フォールバックする（「コミットポリシー → spec 名の決定ルール」参照）。これにより `isCrossRepository` の fail-safe と対称に、title フィールドの異常値も明示的に扱う。
...
- **コミットメッセージ形式**: 1 行目（subject）に**ラウンド番号を必ず含める**。推奨形式は `Address Round {n} review feedback on <spec 名> spec`（`<spec 名>` は対象 PR から決定する — 下記「spec 名の決定ルール」参照）。例: PR タイトルが `Refactor /review-fix to take PR number and delegate to /review skill` なら `<spec 名>` は `/review-fix` となり、コミットメッセージは `Address Round {n} review feedback on /review-fix spec` となる。これにより後から `git log --grep "Round {n}"` でラウンド単位の追跡が可能となり、かつ本 skill を将来別 spec に対して適用する場合も形式が揃う。
    - **spec 名の決定ルール（以下の順序で決定的に適用する）**:
        1. **PR タイトルからスラッシュプレフィックス付き識別子を抽出**: PR タイトルに `/<英数字とハイフン>` 形式（正規表現 `/[A-Za-z][A-Za-z0-9-]*`）の識別子が含まれる場合、**最初に出現する識別子**を spec 名として採用する（例: タイトル `Refactor /review-fix to take PR number and delegate to /review skill` → `/review-fix`）。複数出現する場合も最初の 1 個のみを採用し、順序によって決定論的に定まる。
...
> **Note（同時更新責務）:** Severity 表の**全文記載**（Critical / High / Medium / Low / Nit の 5 行および各レベルの基準・典型例）を持つ箇所は、「使い方 → Reviewer への入力」セクション（Coordinator が Reviewer サブエージェントに渡すプロンプトに含める正本）**のみを唯一の正本**とする。他箇所（「実行例 # 2」のコメント等）は「『Severity レベルの定義』表全文を渡す」のように**ラベル参照（表名参照）に留め、全文を持たない**。これにより本表を変更する場合の同時更新対象は、本セクション（「Severity レベルの定義」表本体）と「使い方 → Reviewer への入力」セクションの**2 箇所のみ**で足り、静的コピーを最小化してバージョンドリフトを防ぐ。新たに Severity 表の全文を別箇所にコピーしてはならない（追加するならラベル参照に留める）。
