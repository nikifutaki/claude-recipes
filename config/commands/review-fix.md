...
- **trim 対象の空白文字は ASCII 範囲の `[ \t\n\r\f\v]`（半角スペース / タブ / LF / CR / FF / VT）のみ。** Unicode 空白（NBSP `U+00A0`、全角空白 `U+3000`、ZWSP `U+200B` 等）は trim 対象外であり、これらが残った文字列は正規表現にマッチせず不正として即エラー扱いとする。実装言語に依存せず一意な挙動を保証する（現時点の実装主体は Claude メインセッションで単一だが、将来スクリプト化や別言語実装に移植する際の挙動一致を保証するため、trim 対象文字集合と正規表現の両方を本 spec で明文化している）。
...
- **(b) PR 情報取得 + 参照可否・種別・state・タイトル取得**
    - `gh pr view <PR番号> --json isCrossRepository,state,headRefName,title` を実行し、`isCrossRepository` / `state` / PR ブランチ名 `<branch>` / PR タイトルをまとめて取得する（※ 可読性のため本節以降では論理的に 4 フィールドをステップ分割して説明するが、実装上は上記のように 1 コールに集約する）。
...
- **Implementer 起動時の入力**: Implementer サブエージェントを起動する際は、`<REVIEW_DIR>/task_{n}.md` の絶対パス・`<REVIEW_DIR>/review_{n}_*.md` の絶対パス・PR 番号・PR ブランチ名（Preflight (b) で取得した `<branch>`）・**PR タイトル**（Preflight (b) の集約コールで取得済み）をプロンプトに含めて渡す。PR タイトルは Implementer がコミットメッセージの `<spec 名>` を抽出するために使用する（「コミットポリシー → spec 名の決定ルール」参照）。Implementer 側での `gh pr view` 再呼び出しを避けるため、Coordinator が取得済みの branch 名・タイトルを再利用する。
...
- **コミットメッセージ形式**: 1 行目（subject）に**ラウンド番号を必ず含める**。推奨形式は `Address Round {n} review feedback on <spec 名> spec`（`<spec 名>` は対象 PR から決定する — 下記「spec 名の決定ルール」参照）。本 PR の場合は `Address Round {n} review feedback on /review-fix spec` となる。これにより後から `git log --grep "Round {n}"` でラウンド単位の追跡が可能となり、かつ本 skill を将来別 spec に対して適用する場合も形式が揃う。
    - **spec 名の決定ルール**: 「`<spec 名>`」は PR タイトルから抽出する（例: PR タイトル「Refactor /review-fix to take PR number and delegate to /review skill」なら `<spec 名>` は `/review-fix`）。PR タイトルから抽出困難な場合は、対象ファイルの主ディレクトリ名を使用してよい（例: `config/commands/<name>.md` が主たる変更対象なら `<spec 名>` は `/<name>`）。Implementer は Coordinator からのプロンプトに PR タイトルが含まれていることを期待し、必要に応じて抽出する（Coordinator の「Implementer 起動時の入力」で PR タイトルは必須項目として渡される）。
...
- **PR 番号**（引数として受け取ったもの）
- **PR ブランチ名**（Coordinator の Preflight (b) で `gh pr view <PR番号> --json isCrossRepository,state,headRefName,title -q .headRefName` 等の経路で取得済みのものをそのまま渡す。Implementer 側での `gh pr view` 再呼び出しは行わず、渡された branch 名を `git symbolic-ref --short HEAD` の出力と照合して防御的に検証するのみとする。詳細は「各ロールの責務 → Implementer」セクション参照）
...
#            「HEAD が detached 状態のため `gh pr checkout <PR番号>` を実行してから再実行」で即中断する。
#            正常終了かつ不一致の場合は「現在 <actual> にチェックアウト中。`gh pr checkout <PR番号>` を実行して `<branch>` に切り替えてから再実行」で即中断する。
...
#             の補足を含める）
#            ※ `<START_HEAD SHA>` は記法上のプレースホルダであり、実行時は
#               `git rev-parse HEAD` で得られた実際の SHA 文字列に置換する
