...
.claude/reviews/            # レビューアーティファクト（プロジェクト内に配置）
  <PR番号>/                 # PR ごとにディレクトリを分離（並行レビュー対応）
    <YYYYMMDD_HHMMSS>/     # 実行ごとにタイムスタンプディレクトリを作成（履歴保持）
      ...
      review_1_c2.md.failed # （参考）失敗検知時の .failed リネーム例
...
**作業ディレクトリ前提（`codex=M>=1` の場合）:** codex は対話的エージェントであり、レビュー時に自前で `git status` / `git diff` / `git diff --check` 等を実行して **現在の作業ディレクトリ** の状態も読み取る。そのため Coordinator は以下を保証すること:

- **レビュー対象 PR のヘッドコミットを HEAD にチェックアウトした状態で codex を起動する**（例: 事前に `git checkout <head>`）。
- 作業ディレクトリに未コミットの変更がある場合は、codex の自発的な `git status` / `git diff` が意図しない差分を拾わないよう、事前にクリーンにするか、その旨をユーザーに警告する。
- codex は diff テキスト（プロンプト内）と作業ディレクトリ状態の **両方** を参照しうる。両者を一致させるのは Coordinator の責任である。
...
**基本形（stdin 方式、推奨。実行時に変数展開する場合）:**

```bash
codex review - < "$REVIEW_DIR/prompt_${n}_c${k}.md" > "$REVIEW_DIR/review_${n}_c${k}.md" 2> "$REVIEW_DIR/review_${n}_c${k}.err"
```

- プレースホルダ版（`<REVIEW_DIR>/prompt_{n}_c{k}.md`）として示す場合も、**実行時はパスをダブルクォートで囲むこと**。`<REVIEW_DIR>` がスペースやシェルメタ文字を含むパスでも安全に動作する。
...
**失敗検知ルール:** codex は非対話的プロセスとして走るため、以下の検知を必ず行い、失敗／不適合の出力は **glob（`review_{n}_*.md`）に拾われないようにする**。codex の実出力は `## Verdict` 見出しを含まないことが多いため、Verdict 行の有無やファイルサイズ閾値には依存せず、次の意味的条件で判定する:
...
3. 失敗・不適合の場合は、当該 `review_{n}_c{k}.md` を **`.failed` 拡張子にリネーム**（例: `review_{n}_c{k}.md.failed`）する。削除は選択肢から外す（`.err` と合わせてデバッグ情報を残すため）。これにより `review_{n}_*.md` glob の対象から外れる。
...
**Claude Reviewer についても同様の原則を適用する:** Task ツールで起動した Claude Reviewer がエラー／タイムアウトで結果を返せなかった場合、Coordinator は具体的に以下の手順を行う:

- Coordinator は **Task ツール戻り値のステータスを確認** し、エラー／タイムアウトを検知する。
- その直後に Bash ツールで `test -f <REVIEW_DIR>/review_{n}_{m}.md && mv <REVIEW_DIR>/review_{n}_{m}.md <REVIEW_DIR>/review_{n}_{m}.md.failed` 相当を実行し、当該出力ファイル（生成されていれば）を `.failed` にリネームする。
- これにより以降の `review_{n}_*.md` glob に拾われないようにする。削除は行わず `.failed` リネームで統一する。
...
# Round 2 以降: Step 1（diff 再生成）から Step 5 までを繰り返し、終了条件を満たすまで継続する。
```
